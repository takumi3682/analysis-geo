...
特定の岩の境界だけ水平線を引いてみる。
特定の土質名のリストに含まれるものの下端境界線だけを朱書きしてみる。
例として「盛土（砂礫）」と「砂礫」の下端を朱書きしてみる。
...
# 2. 作図
# 例：特定の土質名のリストに含まれるものだけ
target_soils <- c("盛土（砂礫）", "砂礫")
...
#             color = "gray40", linetype = "solid", linewidth = 0.3) +
  ...
  geom_hline(
    data = strata_data %>% filter(soil_name %in% target_soils), 
